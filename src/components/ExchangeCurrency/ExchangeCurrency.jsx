...
export default function ExchangeCurrency() {
    const fetchBuySellForex = async () => {
        const response = await instance.get(`/api/Forex/GetForexHomePage`)
        return response?.data
    }

    const useBuySellForex = () => {
        return useQuery({
            queryKey: ["buySellForex"],
            queryFn: fetchBuySellForex,
            select: (data) => data?.buysellforex,
            refetchInterval: false, // Disable automatic interval refetch


        })
    }
    const { data, isLoading, error } = useBuySellForex();
    ...
    return (
        <div className="w-full max-w-3xl mx-auto p-6">
            <div className="bg-white relative rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                {
                    activeTab === 'buying' && <Buying data={data?.buyForex} isLoading={isLoading} />
                }
                {
                    activeTab === 'selling' && <Selling data={data?.sellForex} isLoading={isLoading} />
                }
            </div>
        </div>
    )
